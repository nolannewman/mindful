...
type Props = {
  redirectedFrom: string; // already sanitized
};
...
export default function LoginClient({ redirectedFrom }: Props) {
  ...
  useEffect(() => {
    // When a session appears, ensure profile then route back
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      ...
      options: {
        ...
        shouldCreateUser: true, // user accounts only; providers are seeded
      },
    });
